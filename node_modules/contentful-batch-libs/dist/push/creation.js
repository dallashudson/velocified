...
function createEntities(context, entities, destinationEntities) {
  return _bluebird2.default.map(entities, function (entity) {
    ...
    var operation = destinationEntity ? 'update' : 'create';
...
function createEntry(entry, space, skipContentModel, destinationEntries) {
  ...
  var destinationEntry = getDestinationEntityForSourceEntity(destinationEntries, entry.transformed);
  var operation = destinationEntry ? 'update' : 'create';
  var promise = destinationEntry ? updateDestinationWithSourceData(destinationEntry, entry.transformed) : createEntryInDestination(space, contentTypeId, entry.transformed);
  return promise.then((0, _function.partial)(creationSuccessNotifier, operation)).catch((0, _function.partial)(handleEntryCreationErrors, entry, space, skipContentModel, destinationEntries));
}
